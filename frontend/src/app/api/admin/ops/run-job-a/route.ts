...
/** Proxy POST /api/admin/ops/run-job-a → backend (subprocess Job A). */
export async function POST() {
  ...
  try {
    const response = await fetch(`${apiBaseUrl}/admin/ops/run-job-a`, {
      method: "POST",
    ...
    if (!response.ok) {
      console.error("[admin/ops/run-job-a] Backend error:", response.status, responseBody);
    }

    return NextResponse.json(adaptBackendJsonForProxy(responseBody, response.ok), { status: response.status });
  } catch (error) {
    console.error("[admin/ops/run-job-a] Proxy error:", error);

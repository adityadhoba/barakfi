"""
Public delayed-style Indian equity quotes (NSE website JSON + Yahoo chart API).

Not an official exchange feed. Production apps typically use a licensed vendor
(Groww/Kite/Upstox CMOTS, etc.). These sources may change or rate-limit without notice.
"""
...
def fetch_nse_equity_quote(symbol: str) -> IndianEquityQuote | None:
...
def fetch_yahoo_india_quote(symbol: str, exchange: str = "NSE") -> IndianEquityQuote | None:
...
def fetch_quote_by_provider(symbol: str, exchange: str, provider: str) -> IndianEquityQuote | None:
...
def quote_to_dict(q: IndianEquityQuote) -> dict:
    return {
        "symbol": q.symbol,
        "exchange": q.exchange,
